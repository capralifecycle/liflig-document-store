...
// TODO: update docs
/**
 * A Repository holds the logic for persistence of an [EntityRoot], including lookup methods.
 *
 * The [Dao] owns how an [EntityRoot] is serialized, and it is free to make optimizations so that
 * later lookups will be less costly, e.g. by putting some fields from the [EntityRoot] into
 * specific columns in the persisted storage, which can then be indexed and used for lookup by other
 * [Dao] methods.
 *
 * We want all our methods in the [Dao] to return a [Response] type.
 *
 * For this to work properly, all methods should be wrapped using the [mapExceptionsToResponse]
 * method, which takes care of the exceptions and converts them into proper [Either] types. Note we
 * have no way of consistently guarantee this, so it is left for the developer to remember it.
 */
interface Dao
 ...
 *
 * Note that this does not mean we cannot have more methods, just that we expect these methods for
 * managing persistence of an entity in a consistent way.
 */
...
interface SearchRepository<I, A, Q> where I : EntityId, A : EntityRoot<I> {
...
abstract class AbstractSearchRepository<I, A, Q>(
    ...
    protected val sqlTableName: String,
    protected val serializationAdapter: SerializationAdapter<A>,
) : SearchRepository<I, A, Q> where I : EntityId, A : EntityRoot<I> {
