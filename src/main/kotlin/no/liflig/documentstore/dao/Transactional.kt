...
fun <T> getHandle(jdbi: Jdbi, useHandle: (Handle) -> T): T {
  ...
  return if (transactionHandle != null) {
    ...
    mapExceptions { jdbi.open().use { handle -> useHandle(handle) } }
  }
}

/**
 * Initiates a transaction, and stores the handle in [transactionHandle]. If a transaction is
